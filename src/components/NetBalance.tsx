...
export default function NetBalance() {
  const { configuracion, ingresos, gastosDiarios } = useFinanzas();
  ...
  const metrics = useMemo(() => {
    ...
    const ingresosVarMes = ingresos
      .filter(i => i.tipo === 'variable' && new Date(i.fecha).getTime() >= startOfMonth)
      .reduce((a, i) => a + i.monto, 0);

    const ingresosVarUltMes = ingresos
      .filter(i => i.tipo === 'variable'
        && new Date(i.fecha).getTime() >= startOfLastMonth
        && new Date(i.fecha).getTime() <= endOfLastMonth)
      .reduce((a, i) => a + i.monto, 0);

    const totalIncome = salario + ingresosVarMes;
    const lastMonthIncome = salario + ingresosVarUltMes;
    ...
    const gastosUltMes = gastosDiarios
      .filter(g => new Date(g.fecha).getTime() >= startOfLastMonth
    ...
    const totalExpenses = gastosMes + cuotas + totalSubs;
    const lastMonthExpenses = gastosUltMes + cuotas + totalSubs;

    const net = totalIncome - totalExpenses;
    const lastNet = lastMonthIncome - lastMonthExpenses;
    const pctChange = lastNet !== 0 ? ((net - lastNet) / Math.abs(lastNet)) * 100 : 0;

    return { totalIncome, totalExpenses, net, pctChange, gastosMes, cuotas, totalSubs };
  }, [configuracion, ingresos, gastosDiarios, nextMonthTotal, subscriptions, startOfMonth, startOfLastMonth, endOfLastMonth]);
  ...
  return (
    <div id="net-balance-card" className="glass-panel border-border rounded-2xl p-6 relative overflow-hidden">
      ...
      <div className="flex items-end gap-4 mb-6">
        <p className={`text-5xl font-black tracking-tight ${netColor}`} style={{ filter: metrics.net >= 0 ? 'drop-shadow(0 0 12px hsl(153 100% 50% / 0.4))' : undefined }}>
          {format(metrics.net)}
        </p>
        {pct !== 0 && (
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Gastos diarios', value: metrics.gastosMes, color: 'text-destructive' },
